...
namespace SimpleSimd
{
    public static partial class SimdOps<T>
    {
        public static void Add(in ReadOnlySpan<T> left, T right, in Span<T> result)
        {
            ...
            for (; i < left.Length; i++)
            {
                rResult.Offset(i) = NumOps<T>.Add(rLeft.Offset(i), right);
            }
        }
        ...
        public static void Add(in ReadOnlySpan<T> left, in ReadOnlySpan<T> right, in Span<T> result)
        {
            ...
            if (Vector.IsHardwareAccelerated)
            {
                ...
                for (; i < length; i++)
                {
                    vrResult.Offset(i) = Vector.Add(vrLeft.Offset(i), vrRight.Offset(i));
                }
            ...
            for (; i < left.Length; i++)
            {
                rResult.Offset(i) = NumOps<T>.Add(rLeft.Offset(i), rRight.Offset(i));
            }
        }
